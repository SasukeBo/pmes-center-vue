<template>
  <div class="recent-material">
    ...
    <div class="yield" v-if="material && material.ok && material.ng">
      Yield:
      {{ ((material.ok * 100) / (material.ok + material.ng)).toFixed(2) }}%
    </div>
    <div class="data-block" v-if="material">
      <div class="title">{{ material.customerCode }} ({{ material.name }})</div>
      <div class="sub-title">{{ material.projectRemark }}</div>
      <div class="sub-title">总产量：{{ material.ok + material.ng }}</div>
